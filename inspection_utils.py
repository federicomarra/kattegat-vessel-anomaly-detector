...
def denormalize_predictions(df: pd.DataFrame, metadata_path: str) -> pd.DataFrame:
    ...
    row0 = new_df.iloc[0]
    print(">>> raw normalized real_lon (prima della denorm)")
    print(row0["real_Longitude"])                # o il nome giusto del feat
    print("len:", len(row0["real_Longitude"]))
    print("unique (norm):", sorted(set(row0["real_Longitude"]))[:10])

    i = NUMERIC_COLS.index("Longitude")  # o quello che usi
    m = mean[i]
    s = std[i]
